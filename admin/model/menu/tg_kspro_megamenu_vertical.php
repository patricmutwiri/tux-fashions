...
class ModelMenuTGksproMegamenuVertical extends Model {
	private $errors = array();
	
	public function generate_nestable_list($lang_id) {
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."mega_menu_vertical WHERE parent_id='0' ORDER BY rang");
		$action = $this->url->link('module/tg_kspro_megamenu_vertical', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function menu_showNested($parentID, $lang_id) {	
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."mega_menu_vertical WHERE parent_id='".$parentID."' ORDER BY rang");
		$action = $this->url->link('module/tg_kspro_megamenu_vertical', 'token=' . $this->session->data['token'], 'SSL');
		$output = false;		
	...
	public function save_rang($parent_id, $id, $rang) {
		$this->db->query("UPDATE " . DB_PREFIX . "mega_menu_vertical SET parent_id = '" . $parent_id . "', rang = '" . $rang . "' WHERE id = '" . $id . "'");
	}
	
	public function addMenu($data) {
		$data['content']['categories']['categories'] = json_decode(html_entity_decode($data['content']['categories']['categories']), true);
		$this->db->query("INSERT INTO " . DB_PREFIX . "mega_menu_vertical SET name = '" . $this->db->escape(serialize($data['name'])) . "', description = '" . $this->db->escape(serialize($data['description'])) . "', icon = '" . $data['icon'] . "', link = '" . $data['link'] . "', new_window = '" . $data['new_window'] . "', status = '" . $data['status'] . "', position = '" . $data['position'] . "', submenu_width = '" . $data['submenu_width'] . "', submenu_type = '" . $data['display_submenu'] . "', rang='1000', content_width='" . $data['content_width'] . "', content_type='" . $data['content_type'] . "', content='" . $this->db->escape(serialize($data['content'])) . "'");
	}
	
	public function saveMenu($data) {
		$data['content']['categories']['categories'] = json_decode(html_entity_decode($data['content']['categories']['categories']), true);
		$this->db->query("UPDATE " . DB_PREFIX . "mega_menu_vertical SET name = '" . $this->db->escape(serialize($data['name'])) . "', description = '" . $this->db->escape(serialize($data['description'])) . "', icon = '" . $data['icon'] ."', link = '" . $data['link'] ."', new_window = '" . $data['new_window'] ."', status = '" . $data['status'] ."', position = '" . $data['position'] ."', submenu_width = '" . $data['submenu_width'] ."', submenu_type = '" . $data['display_submenu'] ."', content_width = '" . $data['content_width'] ."', content_type = '" . $data['content_type'] ."', content = '" . $this->db->escape(serialize($data['content'])) . "' WHERE id = '" . $data['id'] . "'");
	}
	
	public function deleteMenu($id) {
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."mega_menu_vertical WHERE id='".$id."'");
		if(count($query->rows) > 0) {
			$query = $this->db->query("SELECT * FROM ".DB_PREFIX."mega_menu_vertical WHERE parent_id='".$id."'");
			if(count($query->rows) > 0) {
				$this->errors[] = "Menu wasn't removed because contains submenu.";
			} else {
				$this->db->query("DELETE FROM " . DB_PREFIX . "mega_menu_vertical WHERE id = '" . $id . "'");
				return true;
			}
		} else {
	...
	public function install() {
		if($this->is_table_exist(DB_PREFIX . "mega_menu_vertical")) {
			$query = $this->db->query("
				CREATE TABLE IF NOT EXISTS `".DB_PREFIX."mega_menu_vertical` (
